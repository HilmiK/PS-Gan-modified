...
class UnetGenerator(nn.Module):
    ...
    def forward(self, input):
        if self.gpu_ids and isinstance(input.data, torch.cuda.FloatTensor):
            #print(input.data.shape,"input")
            #print(self.gpu_ids,"self_gpu")
            #return nn.parallel.data_parallel(self.model, input, self.gpu_ids)
            return self.model(input)
        else:
            return self.model(input)
...
class PersonDiscriminator(nn.Module):
...
class SPP_NET(nn.Module):
    def __init__(self, opt, input_nc, ndf=64, use_sigmoid=False, gpu_ids=[]):
    ...
    def spatial_pyramid_pool(self,previous_conv, num_sample, previous_conv_size, out_pool_size):
        ...
        '''   
        #print(len(out_pool_size)) 
        # print(previous_conv.size())
        for i in range(len(out_pool_size)):
            #print(previous_conv_size)
            #print(i)
            #print(previous_conv.shape,'previous_conv')
            #print(out_pool_size[i],'out_pool_size[i]')
